...
export function Auction(): ReactElement {
  ...
  function handleDeployContract(event: MouseEvent<HTMLButtonElement>) {
    ...
    async function deployDutchAuctionContract(): Promise<void> {
      ...
      try {
        const DutchAuctionContract = await DutchAuctionContractFactory.deploy(reservePrice, numBlocksAuctionOpen, offerPriceDecrement);
    ...
    <>
      ...
      <section style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '30px' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap', alignItems: 'center', gap: '50px' }}>
          <StyledLabel>Reserve Price:</StyledLabel>
          <div>
            {contractDetails?.reservePrice ? contractDetails.reservePrice?.toString() : <em>{`<Not Available>`}</em>}
          </div>
          <StyledLabel>Number of Blocks:</StyledLabel>
          <div>
          ...
          <StyledLabel>Price Decrement:</StyledLabel>
          <div>
            {contractDetails?.offerPriceDecrement ? contractDetails.offerPriceDecrement?.toString() : <em>{`<Not Available>`}</em>}
          </div>
          <StyledLabel>Current Price:</StyledLabel>
          <div>
            {contractDetails?.reservePrice ? contractDetails.currentPrice?.toString() : <em>{`<Not Available>`}</em>}
          </div>
